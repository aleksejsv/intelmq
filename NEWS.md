...
#### Shadowserver Bots
Shadowserver Collector is now able to collect data from [Shadowserver Reports API](https://intelmq.readthedocs.io/en/latest/user/bots.html#shadowserver-reports-api). Moreover Shadowserver Parser now supports JSON.
...
`IntelMQ-Manager` PHP backend is getting replaced by newly developed `IntelMQ-API`. The `IntelMQ-Manager` release only contains the files for web frontend. Install the `IntelMQ-API`, in order to upgrade your local installation. Follow the instructions in our [documentation](https://intelmq.readthedocs.io/en/latest/user/intelmq-api.html) to install `IntelMQ-API` & setup authentication. Unfortunately you can't keep current authentication users, you have to add users as described in the [documentation](https://intelmq.readthedocs.io/en/latest/user/intelmq-api.html#adding-a-user).

**NOTE** If you're using deb/rpm packages, `IntelMQ-API` will automatically be installed if you're upgrading `IntelMQ-Manager`.
